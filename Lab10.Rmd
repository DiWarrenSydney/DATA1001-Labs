...
Suppose we have a coin that we suspect is biased towards heads. Out of 30 tosses we had 21 heads. Using a simulation method with 10000 trials, find an approximate p-value where we have the following null and alternative hypotheses:
$$H_0:P_{heads}\leq0.5$$
$$H_1:P_{heads}>0.5$$
Note: In this scenario, we chose a one sided alternative as we suspect the coin is biased towards *heads*. If we were unsure of the direction of bias, a two sided alternative hypothesis would be appropriate. 

Set the seed to 1 before starting the simulation using the code `set.seed(1)`


<details><summary>Solution</summary>
```{r}
box = c(1,0)    # Box based on our null hypothesis
set.seed(1)     # Set the seed so the results are reproducible

totals = replicate(10000, sum(sample(box, 30, replace = TRUE)))   # Run 1000 trials of 30 coin tosses
p_value = mean(totals >= 21)  # Proportion of trials where the results were as or more extreme than the observed results
p_value
```
</details>
